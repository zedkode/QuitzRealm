...
type MatchSnapshot = {
  ...
  players: Array<{ userId: string; score: number; territoriesWon: number; hasAnswered: boolean; connected: boolean }>;
};
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    socket.on("session:ready", (payload: { activeMatchId?: string | null }) => {
      if (payload.activeMatchId) setNotice("Resuming your active campaign…");
    });
    socket.on("match:state", (payload: MatchSnapshot) => {
      setSnapshot(payload);
      setRoundResult(null);
      setAnswerSubmitted(payload.players.some((player) => player.hasAnswered));
      setBattle(true);
    ...
    socket.on("round:answer-accepted", () => {
      ...
      setNotice("Answer sealed. Awaiting the round resolution.");
    });
    socket.on("round:result", (payload: RoundResult) => {
      setRoundResult(payload);
      setAnswerSubmitted(false);
      setNotice(`Round ${payload.roundNumber} resolved. The correct answer was revealed.`);
    });
  ...
  if (battle) return <Battle mode={current.name} selected={selected} onSelect={setSelected} onAnswer={(answer) => {
    if (!snapshot || answerSubmitted) return;
    socketRef.current?.emit("round:answer", { matchId: snapshot.matchId, answer });
  }} answerSubmitted={answerSubmitted} snapshot={snapshot} roundResult={roundResult} onLeave={() => setBattle(false)} matchStatus={matchStatus} notice={notice} />;
...
function Avatar({ variant = "oracle", color }: { variant?: "astromancer" | "runeknight" | "oracle"; color: string }) { return <div className="relative h-11 w-11 overflow-hidden rounded-full"><HeraldicAvatar variant={variant} className="h-full w-full" /><AvatarRuneRing className={`absolute inset-0 h-full w-full ${color}`} /></div>; }

function Battle({ mode, selected, onSelect, onAnswer, answerSubmitted, snapshot, roundResult, onLeave, matchStatus, notice }: { mode: string; selected: string | null; onSelect: (value: string) => void; onAnswer: (value: string) => void; answerSubmitted: boolean; snapshot: MatchSnapshot | null; roundResult: RoundResult | null; onLeave: () => void; matchStatus: MatchStatus; notice: string }) {
  const answers = snapshot?.question.options ?? [];
  const questionText = snapshot?.question.text ?? "Waiting for the realm to reveal the next question…";
  const players = [{ name: "You", score: "1,842", variant: "runeknight" as const, width: "72%", color: "text-[#e0ba58]" }, { name: "AstraNoir", score: "1,796", variant: "astromancer" as const, width: "68%", color: "text-[#8d6bda]" }, { name: "M. Orpheus", score: "1,721", variant: "oracle" as const, width: "55%", color: "text-[#8d6bda]" }];
  return <div className="realm-surface min-h-[calc(100vh-72px)]"><div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.08fr_1.15fr_290px]">
      <section className="engraved-frame tactical-grid order-2 min-h-[420px] overflow-hidden rounded-2xl p-4 xl:order-1"><div className="relative z-10 flex items-start justify-between"><div><div className="eyebrow">The astral frontier</div><div className="mt-1 text-lg font-bold text-[#f7e7b0]">Territory war table</div></div><div className="rounded-lg border border-[#e0ba58]/25 bg-black/35 px-3 py-2 text-center"><div className="text-[10px] uppercase tracking-widest text-[#a89ca9]">Cycle</div><div className="font-display text-sm text-[#e3c36a]">04 / 08</div></div></div><TerritoryMapSvg className="relative z-10 mt-6 h-auto w-full" /><div className="absolute bottom-5 left-5 right-5 z-10 border border-white/10 bg-[#090a12]/80 p-3 backdrop-blur"><div className="flex justify-between text-xs text-[#aaa0ad]"><span>Map influence</span><b className="text-[#e3c36a]">62%</b></div><div className="mt-2 h-1.5 rounded-full bg-white/8"><div className="h-full w-[62%] rounded-full bg-gradient-to-r from-[#8d6bda] to-[#2bc7b4]" /></div></div></section>
      <section className="order-1 space-y-5 xl:order-2"><div className="engraved-frame rounded-2xl p-6"><div className="flex justify-between gap-3"><div><div className="eyebrow">Lore challenge</div><h2 className="mt-3 max-w-xl font-display text-2xl leading-tight text-[#f7e7b0]">{questionText}</h2></div><div className="hidden text-right sm:block"><div className="flex items-center gap-1 text-[#e3c36a]"><Clock3 size={15} /><span className="font-display text-xl">00:18</span></div><div className="mt-1 text-[10px] uppercase tracking-widest text-[#958a98]">Time to seal</div></div></div><div className="mt-7 grid gap-3">{answers.map((answer, index) => <button key={answer} onClick={() => onSelect(answer)} className={`flex items-center gap-3 rounded-xl border px-4 py-3.5 text-left text-sm ${selected === answer ? "border-[#e0ba58]/60 bg-[#e0ba58]/10 text-[#f7e7b0]" : "border-white/8 bg-black/15 text-[#c0b5c4] hover:border-[#8d6bda]/55 hover:bg-[#8d6bda]/10"}`}><span className="grid h-7 w-7 place-items-center rounded-lg bg-white/7 text-xs font-bold text-[#d6cbe0]">{String.fromCharCode(65 + index)}</span>{answer}{selected === answer && <Check size={16} className="ml-auto text-[#e3c36a]" />}</button>)}</div><button disabled={!selected || answerSubmitted || !snapshot} onClick={() => selected && onAnswer(selected)} className="sigil-button mt-5 flex w-full items-center justify-center gap-2 rounded-lg py-3 text-sm font-extrabold disabled:cursor-not-allowed disabled:opacity-40">{answerSubmitted ? "Answer sealed" : "Seal answer"} <MapPin size={16} /></button></div><div className="panel rounded-2xl p-5"><div className="flex justify-between text-xs text-[#aaa0ad]"><span>Question {snapshot?.roundNumber ?? 0} of {snapshot?.totalRounds ?? 0}</span><span>{roundResult ? `Correct: ${roundResult.correctAnswer}` : `Codex · ${snapshot?.question.categoryId ?? "Awaiting lore"}`}</span></div><div className="mt-3 h-1.5 rounded-full bg-white/8"><div className="h-full w-1/2 rounded-full bg-gradient-to-r from-[#8d6bda] to-[#e0ba58]" /></div></div></section>
      <aside className="order-3 space-y-5"><div className="panel rounded-2xl p-5"><div className="flex items-center justify-between"><span className="eyebrow">Banners</span><span className="text-xs text-[#a7e7de]">LIVE</span></div><div className="mt-4 space-y-4">{players.map((player, index) => <div key={player.name} className="flex items-center gap-3"><Avatar variant={player.variant} color={player.color} /><div className="min-w-0 flex-1"><div className="flex justify-between text-sm"><span className={index === 0 ? "font-bold text-[#f7e7b0]" : "text-[#c3b8c9]"}>{player.name}</span><span className="text-xs text-[#aaa0ad]">{player.score}</span></div><div className="mt-1 h-1 rounded-full bg-white/8"><div className="h-full rounded-full bg-gradient-to-r from-[#8d6bda] to-[#2bc7b4]" style={{ width: player.width }} /></div></div></div>)}</div></div><div className="panel rounded-2xl p-5"><div className="flex items-center gap-2 text-sm font-bold text-[#f7e7b0]"><Sparkles size={16} className="text-[#e3c36a]" /> Battle momentum</div><p className="mt-3 text-sm leading-6 text-[#aaa0ad]">Two true answers in a row. Your next province strike receives <b className="text-[#f1d58d]">+12% force</b>.</p></div></aside>
    </div>
  </div></div>;
}
