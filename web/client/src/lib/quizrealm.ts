...
export const quizRealmApi = {
  ...
  adminPlayers: (query: string) => request<unknown>(`/admin/players${query}`),
  adminPlayerStats: () => request<unknown>("/admin/players/stats"),
  adminPlayerDetail: (id: string) => request<unknown>(`/admin/players/${id}`),
  adminRevealEmail: (id: string) => request<{ email: string }>(`/admin/players/${id}/reveal-email`, { method: "POST" }),
  adminPlayersBulk: (action: string, ids: string[]) => request<unknown>("/admin/players/bulk", { method: "POST", body: JSON.stringify({ action, ids }) }),
