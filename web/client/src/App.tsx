        ...
        <header className="sticky top-0 z-40 border-b border-[#d9b45a]/10 bg-[#07060b]/85 backdrop-blur-xl">
          <div className="mx-auto flex h-18 max-w-[1440px] items-center justify-between px-5 lg:px-10">
            <Link href="/" className="flex items-center gap-3">
              ...
              <div><div className="font-display text-sm tracking-wide text-[#f7e7b0]">QUIZREALM</div><div className="eyebrow !text-[.52rem]">The arcane arena</div></div>
            </Link>
            <nav className="hidden items-center gap-1 md:flex">
              ...
              {user?.role === "admin" && <Link href="/admin" className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm ${location.startsWith("/admin") ? "bg-[#7e5cc7]/15 text-[#c7b3ff]" : "text-[#a69ead] hover:bg-white/5 hover:text-white"}`}><Shield size={16} />Admin</Link>}
            </nav>
            <div className="flex items-center gap-3">
              {user ? <button onClick={logout} className="hidden rounded-lg border border-white/10 px-3 py-2 text-xs text-[#a69ead] hover:border-[#d9b45a]/30 hover:text-white sm:block">Sign out</button> : <Link href="/auth" className="flex items-center gap-2 rounded-lg bg-[#d9b45a] px-4 py-2 text-sm font-bold text-[#1a1207] hover:bg-[#f0ce76]"><LogIn size={16} />Enter the realm</Link>}
            </div>
          </div>
        </header>
      )}
      <main>{children}</main>
      {!isHome && <footer className="border-t border-white/5 py-8 text-center text-xs text-[#6f6875]">QuizRealm · Knowledge becomes territory.</footer>}
    </div>
  );
}
...
function AdminGate() {
  ...
  if (state.loading) return <div className="starfield grid min-h-[70vh] place-items-center text-sm text-[#a69ead]">Verifying realm clearance…</div>;
  if (!state.user || state.user.role !== "admin") return <div className="starfield grid min-h-[70vh] place-items-center px-5"><div className="panel max-w-md rounded-2xl p-8 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-xl bg-[#c7779b]/12 text-[#e596a8]"><Shield size={20}/></div><h1 className="mt-5 font-display text-2xl text-[#f7e7b0]">The council bars your path</h1><p className="mt-3 text-sm leading-6 text-[#a69ead]">Admin clearance is verified by the existing QuizRealm backend. Sign in with an administrator account to enter the control room.</p><Link href="/auth" className="mt-6 inline-flex rounded-xl bg-[#d9b45a] px-5 py-3 text-sm font-bold text-[#1a1207]">Sign in as admin</Link></div></div>;
  return <Admin />;
}
